...
for(i in 1:nrow(df)){
    df[i,5] = calculate.score(df[i,1], df[i,2], df[i,3])
}
(df)
```
...
*Part j*
Part j is not vectorizable. If/else statements are not vectorized which makes passing through a vector not work, only taking in the first value. However, you can, as we did, pass a vector through a function using the if/else statement, but on its own it is not vectorizable.
...
**Solution**
```{r}
img.src = html_attr(image.element, "src")
image.url = paste("website/", img.src, sep = "")
```
